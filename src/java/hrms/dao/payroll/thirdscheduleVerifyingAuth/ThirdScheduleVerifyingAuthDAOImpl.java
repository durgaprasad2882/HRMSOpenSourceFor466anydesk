...
import hrms.model.payroll.thirdscheduleverifyingauth.ThirdScheduleVerifyingAuthBean;
...
public class ThirdScheduleVerifyingAuthDAOImpl implements ThirdScheduleVerifyingAuthDAO {
    ...
    public List getEmployeeList(String empid, int rows, int page, String offcode) {
        ...
        ThirdScheduleVerifyingAuthBean tbean = null;
        ...
        try {
            ...
            if (offcode != null && !offcode.equals("")) {
                offcode = offcode.replaceAll(",", "");
                sql = "SELECT PAY_REVISION_OPTION.EMP_ID,GPF_NO,ARRAY_TO_STRING(ARRAY[INITIALS, F_NAME, M_NAME,L_NAME], ' ') FULL_NAME,G_POST.POST,CUR_SALARY,PAY_REVISION_OPTION.PAY_SCALE,PAY_REVISION_OPTION.GP,is_approved_verifying_auth,pay_revision_option.gpc,entry_gpc FROM PAY_REVISION_OPTION"
                        + " INNER JOIN EMP_MAST ON PAY_REVISION_OPTION.EMP_ID=EMP_MAST.EMP_ID"
                        + " LEFT OUTER JOIN G_SPC ON EMP_MAST.CUR_SPC=G_SPC.SPC"
                        + " LEFT OUTER JOIN G_POST ON G_SPC.GPC=G_POST.POST_CODE WHERE PAY_REVISION_OPTION.verifying_auth_emp_id=? and PAY_REVISION_OPTION.off_code=? ORDER BY F_NAME,L_NAME ASC LIMIT ? OFFSET ?";
                ...
                sql = "SELECT PAY_REVISION_OPTION.EMP_ID,GPF_NO,ARRAY_TO_STRING(ARRAY[INITIALS, F_NAME, M_NAME,L_NAME], ' ') FULL_NAME,G_POST.POST,CUR_SALARY,PAY_REVISION_OPTION.PAY_SCALE,PAY_REVISION_OPTION.GP,is_approved_verifying_auth,pay_revision_option.gpc,entry_gpc FROM PAY_REVISION_OPTION"
                        + " INNER JOIN EMP_MAST ON PAY_REVISION_OPTION.EMP_ID=EMP_MAST.EMP_ID"
                        + " LEFT OUTER JOIN G_SPC ON EMP_MAST.CUR_SPC=G_SPC.SPC"
                        + " LEFT OUTER JOIN G_POST ON G_SPC.GPC=G_POST.POST_CODE WHERE PAY_REVISION_OPTION.verifying_auth_emp_id=? ORDER BY F_NAME,L_NAME ASC LIMIT ? OFFSET ?";
                pst = con.prepareStatement(sql);
                pst.setString(1, empid);
            ...
            rs = pst.executeQuery();
            while (rs.next()) {
                tbean = new ThirdScheduleVerifyingAuthBean();
                ...
                tbean.setIsApproved(StringUtils.defaultString(rs.getString("is_approved_verifying_auth")));
                emplist.add(tbean);
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ...
            if (offcode != null && !offcode.equals("")) {
                ...
                sql = "SELECT count(PAY_REVISION_OPTION.EMP_ID) CNT FROM PAY_REVISION_OPTION"
                        + " INNER JOIN EMP_MAST ON PAY_REVISION_OPTION.EMP_ID=EMP_MAST.EMP_ID"
                        + " WHERE PAY_REVISION_OPTION.verifying_auth_emp_id=? and PAY_REVISION_OPTION.off_code=?";
                pst = con.prepareStatement(sql);
                pst.setString(1, empid);
                pst.setString(2, offcode);
            } else {
                sql = "SELECT count(PAY_REVISION_OPTION.EMP_ID) CNT FROM PAY_REVISION_OPTION"
                        + " INNER JOIN EMP_MAST ON PAY_REVISION_OPTION.EMP_ID=EMP_MAST.EMP_ID"
                        + " WHERE PAY_REVISION_OPTION.verifying_auth_emp_id=?";
                pst = con.prepareStatement(sql);
                pst.setString(1, empid);
            }
            rs = pst.executeQuery();
    ...
    public void thirdSchedulePDF(Document document, String empid) {
        ...
        try {
            ...
            String sql = "select EMP_ID,officiating, EMP_NAME,PAY_REVISION_OPTION.department_code,hoo_spc, g_post2.post hoo_post_name,cur_basic_salary,CUR_SALARY,CUR_GP,PAY_REVISION_OPTION.POST,option_chosen,cur_post_paymatrix_level,entry_gpc,entry_paymatrix_level,nos_racp_availed,nos_promotion_availed,nos_racp_before_promotion,nos_racp_after_promotion,basicpay_fix_paymatrix,basicpay_fix_paymatrix_date,"
                    ...
                    + " PAY_REVISION_OPTION.basic,ddoPayScale,ddoGP,revised_basic,da,is_approved_verifying_auth,revision_pay_scale,"
            ...
            if (rs.next()) {
                ...
                if (revisedList != null && revisedList.size() > 0) {
                    ...
                    while (incrItr.hasNext()) {
                        tbean = (ThirdScheduleVerifyingAuthBean) incrItr.next();
                        if (tbean.getPp() > 0) {
                ...
                for (int i = 0; i < revisedList.size(); i++) {
                    tbean = (ThirdScheduleVerifyingAuthBean) revisedList.get(i);
    ...
    public ThirdScheduleVerifyingAuthForm getThirdScheduleData(String empid) {
        ...
        ThirdScheduleVerifyingAuthForm tform = new ThirdScheduleVerifyingAuthForm();
        ...
        try {
            ...
            if (rs.next()) {
                ...
                tform.setIsApproved(rs.getString("is_approved_verifying_auth"));
    ...
    public Message revertVerifyingAuthEmp(String chkEmp) {
        ...
        try {
            ...
            String sql = "UPDATE PAY_REVISION_OPTION SET is_submitted_verifying_auth=?,verifying_auth_emp_id=?,verifying_auth_spc=?,is_approved_verifying_auth=?,is_approved_checking_auth=? WHERE EMP_ID=?";
            pst = con.prepareStatement(sql);
    ...
    public void thirdScheduleIASPDF(Document document, ThirdScheduleVerifyingAuthForm tform) {
        ...
        try {
            ...
            if (revisedList.size() > 0) {
                tbean = (ThirdScheduleVerifyingAuthBean) revisedList.get(0);
            }
    ...
    public List getOfficeList(String empid) {
        ...
        try {
            ...
            String sql = "SELECT OFF_EN,PAY_REVISION_OPTION.OFF_CODE FROM (SELECT OFF_CODE FROM PAY_REVISION_OPTION WHERE verifying_auth_emp_id=? GROUP BY OFF_CODE)PAY_REVISION_OPTION"
