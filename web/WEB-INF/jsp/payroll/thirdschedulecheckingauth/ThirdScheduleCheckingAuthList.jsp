        ...
        <script type="text/javascript">
            $(document).ready(function() {
                ...
                $('#emplistthirdscheduledg').datagrid({
                    url: "getCheckingAuthThirdScheduleEmpListJSON.htm?offcode=",
            ...
            function viewThirdSchedule() {
                ...
                if (row) {
                    ...
                    } else if (row.isVerifyingAuthSubmitted == "Y") {
                        alert("Already Submitted");
                    } else {
                        $('#winThirdScheduleAuth').window('refresh', 'viewCheckingAuthThirdSchedule.htm?empid=' + row.empId);
            ...
            function viewThirdSchedulePDF() {
                ...
                if (row) {
                    ...
                    if (rows.length > 1) {
                        ...
                        window.open("ThirdscheduleCheckingAuthPDF.htm?empid=" + row.empId, "_blank");
                    }
                } else {
                    alert("Select a row");
                }
            ...
            function saveThirdSchedule(isApproved) {
                ...
                if (status) {
                    $('#thirdScheduleDataForm').form('submit', {
                        url: 'saveThirdCheckingAuthScheduleData.htm?approve=' + isApproved,
            ...
            function openVerifyingAuth() {
                var row = $('#emplistthirdscheduledg').datagrid('getSelected');
                var isApproved = "Y";
                var isVerifyingAuthSubmitted = "N";
                if (row) {
                    var rows = $('#emplistthirdscheduledg').datagrid('getSelections');
                    for (var i = 0; i < rows.length; i++) {
                        ...
                        if (rows[i].isVerifyingAuthSubmitted == 'Y') {
                            isVerifyingAuthSubmitted = "Y";
                        }
                    }
                    if (isApproved == "N") {
                        alert("One of the Selected Employees are not Approved!");
                    } else if (isVerifyingAuthSubmitted == "Y") {
                        alert("One of the Selected Employees are already Submitted to Verifying Authority!");
                    } else {
                        $('#winVerifyingAuth').window("open");
                    }
                } else {
                    alert("Select a Row");
                }
            ...
            function openRevert() {
                ...
                if (row) {
                    ...
                    $('#chkEmp').val(ids);
                    if (isVerifyingAuthSubmitted == "Y") {
                        alert("One of the selected Employees are already submitted to Verifying Authority and cannot be reverted!");
                        $('#chkEmp').val('');
                    } else if (confirm("Are you sure to Revert the selected Employees?")) {
                        $('#authSubmitForm').form('submit', {
                            url: 'revertCheckingAuthEmp.htm',
                            success: function(result) {
                                var result = eval('(' + result + ')');
                                if (result.message) {
                                    $.messager.show({
    ...
    <body>
        <div align="center" style="margin-top:20px;">
            <table border="0" cellpadding="0" cellspacing="0" width="80%">
                <tr style="height:40px;">
                    ...
                    <td width="70%">
                        <input name="sltOffCode" id="sltOffCode" class="easyui-combobox" style="width:500px;" data-options="url:'getCheckingEmployeesOfficeListJSON.htm',valueField:'value',textField:'label',editable:false" />&emsp;
        ...
        <div align="center">
            <table id="emplistthirdscheduledg" class="easyui-datagrid" style="width:100%;height:400px;overflow:auto;" title="Employye List"
                ...
                <thead>
                    <tr>
                        <th data-options="field:'temp',checkbox:'true',styler:function(index,row){if (row.isApproved != 'Y' || row.isVerifyingAuthSubmitted == 'Y'){return {class:'dg-nocheck'};}}"></th>
                        ...
                        <th data-options="field:'isVerifyingAuthSubmitted',width:80,formatter:showStatus">Status</th>
                        <th data-options="field:'verifyingAuthName',width:80,formatter:showPendingAuth">Pending At</th>
                    </tr> 
                </thead>
            </table>
        </div>
        ...
        <div id="emplistToolbar" style="padding:3px">                    
            ...
            <a href="javascript:openVerifyingAuth();" class="easyui-linkbutton" iconCls="icon-add" plain="true">Submit to Verifying Authority</a>
        ...
        <div id="winVerifyingAuth" class="easyui-window" title="Verifying Authority" style="width:700px;height:400px;top:50px;padding:10px 20px" closed="true" buttons="#searchdlg-buttons"
